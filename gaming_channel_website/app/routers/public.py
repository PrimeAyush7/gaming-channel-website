...
async def media_file_view(media_id: int):
    media = media_service.get_media_bytes(media_id)
    if not media or media.get("data") is None:
        raise HTTPException(status_code=404, detail="Media not found")

    response = Response(content=bytes(media["data"]), media_type=media["mime_type"] or "application/octet-stream")
