...
async def media_file_view(media_id: int):
    """Serve uploaded media from persistent PostgreSQL storage.

    New uploads are stored as BYTEA, so this endpoint remains functional even
    when Render's ephemeral local filesystem is recreated.
    """
    media = media_service.get_media_content(media_id)
    if not media:
        raise HTTPException(status_code=404, detail="Media not found")

    response = Response(content=media["data"], media_type=media["mime_type"])
