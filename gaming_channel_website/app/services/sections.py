...
def get_section_by_slug(slug: str):
    with get_db() as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM sections WHERE slug = ?;", (slug.strip(),))
        row = cursor.fetchone()
        return dict(row) if row else None
...
def create_section(name: str, slug: str, description: str = "", icon: str = "🎮", is_nav_visible: bool = True, sort_order: int = 0):
    ...
    with get_db() as conn:
        cursor = conn.cursor()
        # Ensure unique slug
        cursor.execute("SELECT id FROM sections WHERE slug = ?;", (slug,))
        if cursor.fetchone():
            count = 1
            orig_slug = slug
            while True:
...
def update_section(section_id: int, name: str, slug: str, description: str = "", icon: str = "🎮", is_nav_visible: bool = True, sort_order: int = 0):
    ...
    with get_db() as conn:
        ...
        cursor.execute("SELECT id FROM sections WHERE slug = ? AND id != ?;", (slug, section_id))
        if cursor.fetchone():
            slug = f"{slug}-{section_id}"
            
        cursor.execute("""
            UPDATE sections
            SET name = ?, slug = ?, description = ?, icon = ?, is_nav_visible = ?, sort_order = ?
            WHERE id = ?;
