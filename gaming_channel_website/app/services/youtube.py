...
def add_video(title: str, youtube_input: str, is_featured: bool = False, sort_order: int = 0):
    ...
    with get_db() as conn:
        ...
        res = cursor.fetchone()
        vid_id = res["id"] if isinstance(res, dict) or hasattr(res, "__getitem__") else res[0]
        return vid_id, None
...
def update_video(video_id: int, title: str, youtube_input: str, is_featured: bool = False, sort_order: int = 0):
    ...
    with get_db() as conn:
        cursor = conn.cursor()
        if is_featured:
            cursor.execute("UPDATE youtube_videos SET is_featured = 0 WHERE id != %s;", (video_id,))
        cursor.execute("""
            UPDATE youtube_videos
            SET title = %s, youtube_id = %s, is_featured = %s, sort_order = %s
            WHERE id = %s;
