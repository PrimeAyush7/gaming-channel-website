...
- **Persistent Media Storage:** Uploaded image binaries are stored in PostgreSQL (`media.data` as `BYTEA`) and served through `/media/{id}`, so new media survives Render restarts, redeploys, and sleep cycles. A local copy may be written for compatibility, but it is never the authoritative source.
