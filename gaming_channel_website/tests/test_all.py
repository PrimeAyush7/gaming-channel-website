...
try:
    ...
    class _MockCursor:
        ...
        def execute(self, query, params=None):
            ...
            if re.search(r'information_schema\.columns', sql, flags=re.IGNORECASE):
                # Map the production column-existence query to SQLite PRAGMA data.
                m = re.search(r"column_name\s*=\s*'([^']+)'", sql, flags=re.IGNORECASE)
                t = re.search(r"table_name\s*=\s*'([^']+)'", sql, flags=re.IGNORECASE)
                if m and t:
                    cols = self._cur.execute(f"PRAGMA table_info({t.group(1)})").fetchall()
                    found = any(row[1] == m.group(1) for row in cols)
                    self._last_id = None
                    self._ret_col = None
                    self._static_rows = [(1,)] if found else []
                    return self
...
class TestNexusGamingPortal(unittest.TestCase):
    ...
    def test_18_media_persists_without_local_filesystem(self):
        """New media must remain servable after its Render-local file disappears."""
        from app.services import media as media_service

        img_byte_arr = io.BytesIO()
        Image.new('RGB', (32, 32), color='orange').save(img_byte_arr, format='PNG')
        img_bytes = img_byte_arr.getvalue()

        record, error = media_service.save_media_file(
            "persistent_test.png", img_bytes, "image/png"
        )
        self.assertIsNone(error)
        self.assertIsNotNone(record)
        self.assertTrue(record["url"].startswith("/media/"))

        media_id = record["id"]
        local_path = UPLOAD_DIR / record["filename"]
        if local_path.exists():
            local_path.unlink()

        # The image must still be served entirely from PostgreSQL.
        response = self.client.get(record["url"])
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.headers.get("content-type"), "image/png")
        self.assertEqual(response.content, img_bytes)
        self.assertIn("max-age=31536000", response.headers.get("cache-control", ""))

        # Re-running initialization must not remove the stored binary.
        init_db()
        response_after_init = self.client.get(f"/media/{media_id}")
        self.assertEqual(response_after_init.status_code, 200)
        self.assertEqual(response_after_init.content, img_bytes)
