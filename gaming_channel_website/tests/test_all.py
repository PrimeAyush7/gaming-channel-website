...
try:
    ...
    class _MockCursor:
        ...
        def execute(self, query, params=None):
            ...
            # Small PostgreSQL compatibility shim used by init_db() to detect
            # whether a table existed before startup.
            if re.search(r"SELECT\s+to_regclass\('public\.sections'\)\s+IS\s+NOT\s+NULL", sql, re.IGNORECASE):
                exists = self._cur.execute(
                    "SELECT 1 FROM sqlite_master WHERE type='table' AND name='sections' LIMIT 1"
                ).fetchone() is not None
                self._special_result = [{"table_exists": 1 if exists else 0}]
                self._ret_col = None
                return self

            self._special_result = None
        ...
        def fetchone(self):
            if self._special_result is not None:
                result = self._special_result[0] if self._special_result else None
                self._special_result = None
                return result
...
class TestNexusGamingPortal(unittest.TestCase):
    ...
    def test_14_deleted_dynamic_section_survives_restart_initialization(self):
        # Remove one of the built-in sections, simulate an application restart by
        # running init_db() again, and ensure the deleted section is not recreated.
        section = section_service.get_section_by_slug("events")
        self.assertIsNotNone(section)

        section_service.delete_section(section["id"])
        self.assertIsNone(section_service.get_section_by_slug("events"))

        init_db()

        self.assertIsNone(
            section_service.get_section_by_slug("events"),
            "Deleted dynamic sections must not be recreated on application startup.",
        )

    def test_15_post_crud_and_download_system(self):
        cookie, csrf_token = self.login_admin()
