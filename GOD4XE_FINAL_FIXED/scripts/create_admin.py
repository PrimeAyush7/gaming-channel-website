...
def main():
    init_db()
    parser = argparse.ArgumentParser(description="Create or update an admin user for GOD4XE Gaming portal.")
    ...
    with get_db() as conn:
        ...
        if existing:
            admin_id = existing["id"] if isinstance(existing, dict) or hasattr(existing, "__getitem__") else existing[0]
            cursor.execute("""
            ...
            cursor.execute("""
                INSERT INTO admins (username, password_hash, salt) VALUES (%s, %s, %s);
