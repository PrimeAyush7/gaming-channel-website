...
def verify_csrf(admin: dict, token: str):
    if not token or token != admin.get("csrf_token"):
        raise HTTPException(status_code=403, detail="Invalid CSRF token")


# TOURNAMENT CATEGORIES & GUNS
# --------------------------------------------------------------------------
@router.get("/categories", response_class=HTMLResponse)
async def admin_categories(request: Request, admin: dict = Depends(get_current_admin)):
    cats = tournament_service.list_categories(only_active=False)
...
async def admin_create_category(
    ...
    tournament_service.create_category(
        name=name, slug=slug, description=description, image_url=image_url, display_order=display_order
    )
    return RedirectResponse(url="/admin/categories", status_code=status.HTTP_303_SEE_OTHER)

@router.post("/categories/delete/{cat_id}")
async def admin_delete_category(cat_id: int, csrf_token: str = Form(...), admin: dict = Depends(get_current_admin)):
    verify_csrf(admin, csrf_token)
    tournament_service.delete_category(cat_id)
    return RedirectResponse(url="/admin/categories", status_code=status.HTTP_303_SEE_OTHER)
...
async def admin_create_subcategory(
    ...
    verify_csrf(admin, csrf_token)
    tournament_service.create_subcategory(
        category_id=category_id, name=name, slug=slug, description=description, image_url=image_url, display_order=display_order
    )
    return RedirectResponse(url="/admin/categories", status_code=status.HTTP_303_SEE_OTHER)

@router.get("/guns", response_class=HTMLResponse)
async def admin_guns(request: Request, admin: dict = Depends(get_current_admin)):
    guns = tournament_service.list_guns(only_active=False)
    return templates.TemplateResponse("admin/guns.html", {
        "request": request,
        "admin": admin,
        "active_nav": "guns",
        "guns": guns,
...
@router.post("/guns/new")
async def admin_create_gun(
    name: str = Form(...),
    code: str = Form(...),
    image_url: str = Form(None),
    display_order: int = Form(0),
    ...
    verify_csrf(admin, csrf_token)
    tournament_service.create_gun(name=name, code=code, image_url=image_url, display_order=display_order)
    return RedirectResponse(url="/admin/guns", status_code=status.HTTP_303_SEE_OTHER)

@router.post("/guns/delete/{gun_id}")
async def admin_delete_gun(gun_id: int, csrf_token: str = Form(...), admin: dict = Depends(get_current_admin)):
    verify_csrf(admin, csrf_token)
    tournament_service.delete_gun(gun_id)
    return RedirectResponse(url="/admin/guns", status_code=status.HTTP_303_SEE_OTHER)


# --------------------------------------------------------------------------
# TOURNAMENTS EXTENSIONS (COMPLETE EARLY, PARTICIPANTS, RESULTS)
# --------------------------------------------------------------------------
...
async def admin_tournament_participants(tournament_id: int, request: Request, admin: dict = Depends(get_current_admin)):
    ...
    return templates.TemplateResponse("admin/tournament_participants.html", {
        "request": request,
        "admin": admin,
        "active_nav": "tournaments",
        "tournament": t,
...
@router.get("/tournaments/{tournament_id}/results", response_class=HTMLResponse)
async def admin_tournament_results_view(tournament_id: int, request: Request, admin: dict = Depends(get_current_admin)):
    t = tournament_service.get_tournament_by_id(tournament_id)
    parts = tournament_service.list_tournament_participants(tournament_id)
    results = tournament_service.get_tournament_results(tournament_id)
    return templates.TemplateResponse("admin/tournament_results.html", {
        "request": request,
        "admin": admin,
        "active_nav": "tournaments",
        "tournament": t,
...
async def admin_tournament_results_submit(tournament_id: int, request: Request, admin: dict = Depends(get_current_admin)):
    form = await request.form()
    verify_csrf(admin, form.get("csrf_token", ""))
    proof_url = form.get("proof_url", "")
    p_count = int(form.get("participant_count", 0) or 0)
    
    results = []
    for i in range(1, p_count + 1):
        u_id = form.get(f"user_id_{i}")
        if u_id:
            results.append({
                "user_id": int(u_id),
                "ff_uid": form.get(f"ff_uid_{i}", ""),
                "ff_ign": form.get(f"ff_ign_{i}", ""),
                "placement": int(form.get(f"placement_{i}", i) or i),
                "kills": int(form.get(f"kills_{i}", 0) or 0),
                "placement_prize_diamonds": int(form.get(f"placement_prize_{i}", 0) or 0)
            })
            
    tournament_service.enter_tournament_results(
...
async def admin_users(
    ...
    users, total = user_service.list_app_users(search=q, provider_filter=provider, status_filter=status, limit=100)
...
async def admin_withdrawals(request: Request, status: str = None, admin: dict = Depends(get_current_admin)):
    withs = wallet_service.list_all_withdrawal_requests(status_filter=status, limit=100)
...
# --------------------------------------------------------------------------
# LATEST UPDATES & ANNOUNCEMENTS & COMMUNITIES & TUTORIALS & DISPUTES
# --------------------------------------------------------------------------
@router.get("/updates", response_class=HTMLResponse)
async def admin_updates(request: Request, admin: dict = Depends(get_current_admin)):
    updates = content_features.list_latest_updates(only_published=False)
...
async def admin_create_update(
    ...
    youtube_video_url: str = Form(None),
    ...
    verify_csrf(admin, csrf_token)
    content_features.create_latest_update(
        heading=heading, description=description, cover_image_url=cover_image_url,
        youtube_video_url=youtube_video_url, button_text=button_text, button_url=button_url, display_order=display_order
    )
    return RedirectResponse(url="/admin/updates", status_code=status.HTTP_303_SEE_OTHER)

@router.post("/updates/delete/{up_id}")
async def admin_delete_update(up_id: int, csrf_token: str = Form(...), admin: dict = Depends(get_current_admin)):
    verify_csrf(admin, csrf_token)
    content_features.delete_latest_update(up_id)
    return RedirectResponse(url="/admin/updates", status_code=status.HTTP_303_SEE_OTHER)

@router.get("/announcements", response_class=HTMLResponse)
async def admin_announcements(request: Request, admin: dict = Depends(get_current_admin)):
    anns = content_features.list_announcements(only_active=False)
...
@router.post("/announcements/notice")
async def admin_save_notice(arena_notice: str = Form(...), csrf_token: str = Form(...), admin: dict = Depends(get_current_admin)):
    verify_csrf(admin, csrf_token)
    settings_service.update_setting("arena_notice_marquee", arena_notice.strip())
    return RedirectResponse(url="/admin/announcements", status_code=status.HTTP_303_SEE_OTHER)

@router.post("/announcements/new")
async def admin_create_ann(title: str = Form(...), message: str = Form(...), type: str = Form("INFO"), csrf_token: str = Form(...), admin: dict = Depends(get_current_admin)):
    verify_csrf(admin, csrf_token)
    content_features.create_announcement(title=title, message=message, type=type)
    return RedirectResponse(url="/admin/announcements", status_code=status.HTTP_303_SEE_OTHER)

@router.post("/announcements/delete/{a_id}")
async def admin_delete_ann(a_id: int, csrf_token: str = Form(...), admin: dict = Depends(get_current_admin)):
    verify_csrf(admin, csrf_token)
    content_features.delete_announcement(a_id)
    return RedirectResponse(url="/admin/announcements", status_code=status.HTTP_303_SEE_OTHER)

@router.get("/disputes", response_class=HTMLResponse)
async def admin_disputes(request: Request, status: str = None, admin: dict = Depends(get_current_admin)):
    disps = tournament_service.list_tournament_disputes(status_filter=status, limit=100)
...
async def admin_tutorials(request: Request, admin: dict = Depends(get_current_admin)):
    tuts = content_features.list_tutorials(only_published=False)
    return templates.TemplateResponse("admin/tutorials.html", {
        "request": request,
        "admin": admin,
...
async def admin_create_tutorial(
    ...
    verify_csrf(admin, csrf_token)
    content_features.create_tutorial(
        title=title, slug=slug, category=category, content=content, image_url=image_url, video_url=video_url
    )
    return RedirectResponse(url="/admin/tutorials", status_code=status.HTTP_303_SEE_OTHER)

@router.post("/tutorials/delete/{t_id}")
async def admin_delete_tutorial(t_id: int, csrf_token: str = Form(...), admin: dict = Depends(get_current_admin)):
    verify_csrf(admin, csrf_token)
    content_features.delete_tutorial(t_id)
    return RedirectResponse(url="/admin/tutorials", status_code=status.HTTP_303_SEE_OTHER)

@router.get("/communities", response_class=HTMLResponse)
async def admin_communities(request: Request, admin: dict = Depends(get_current_admin)):
    comms = content_features.list_communities(only_active=False)
...
@router.post("/communities/edit/{c_id}")
async def admin_edit_community(
    c_id: int,
    platform: str = Form(...),
    name: str = Form(...),
    url: str = Form(...),
