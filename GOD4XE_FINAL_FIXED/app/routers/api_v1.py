...
async def api_google_auth(req: GoogleAuthRequest):
    try:
        user, token = user_service.authenticate_google_user(req.id_token)
...
async def api_referrals(user: dict = Depends(get_auth_user)):
    stats = social_service.get_referral_stats(user["id"])
