...
def register_user(
    ...
    with get_db() as conn:
        ...
        # Initialize diamond ledger account with 25 free starter diamonds
        cursor.execute("""
        ...
        starter_tx_uuid = str(uuid.uuid4())
        cursor.execute("""
            INSERT INTO diamond_transactions (
                transaction_uuid, user_id, amount, balance_after,
                tx_type, reference_id, description
            ) VALUES (%s, %s, 25, 25, 'STARTER_BONUS', %s, %s);
        """, (starter_tx_uuid, user_id, f"STARTER-{user_id}", "25 Free Starter Diamonds"))

        # Track referral if present
        if referrer_id and referrer_id != user_id:
            cursor.execute("""
                INSERT INTO referrals (referrer_id, referee_id, reward_diamonds, status)
                VALUES (%s, %s, 25, 'PENDING');
            """, (referrer_id, user_id))
...
def authenticate_google_user(id_token_str: str) -> tuple[dict, str]:
    ...
    with get_db() as conn:
        ...
        if existing:
            ...
            """, (starter_tx_uuid, user_id, f"STARTER-GOOGLE-{user_id}", "25 Free Starter Diamonds"))
...
def verify_phone_otp(phone: str, otp_code: str) -> tuple[dict, str]:
    ...
    with get_db() as conn:
        ...
        if user_row:
            ...
            """, (starter_tx_uuid, user_id, f"STARTER-OTP-{user_id}", "25 Free Starter Diamonds"))
